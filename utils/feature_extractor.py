...
Extraccion de caracteristicas acusticas de sonidos cardiacos.
Combina MFCC, Mel-Spectrograma, ZCR, RMS y metadatos del JSON clinico.
"""
import io
import logging
...
def extract_audio_features(y: np.ndarray) -> np.ndarray:
    """
    Extrae un vector de caracteristicas del audio.

    Retorna un vector 1-D de dimension fija con:
      - MFCC (mean + std)  -> 2xN_MFCC
...
def extract_metadata_features(metadata: dict) -> np.ndarray:
    """
    Convierte el JSON clinico en un vector numerico.
    ...
    Campos usados:
      ...
      - enfermedades_base (si esta presente, binario)
    """
    feats = []
    ...
    # Foco de auscultacion (importante clinicamente)
    foco_map = {
        "Aortico": 0,
        "Aortico": 0,
        "Pulmonar": 1,
    ...
    # Codigo de hospital y consultorio como numerico
...
def build_full_feature_vector(
    ...
    - Caracteristicas del audio principal
    - Caracteristicas del audio ECG (si existe)
    - Metadatos clinicos
    """
    ...
    if y_ecg is not None:
        ...
        # Diferencia energetica entre senales (rasgo clinico)
