...
Maneja descarga, subida y listado de archivos.
"""
import json
import logging
...
def download_audio_json(filename: str) -> Optional[dict]:
    """Descarga y parsea un JSON clinico desde el servidor local."""
    raw = download_file(REMOTE_FOLDERS["audio_json"], filename)
    if raw:
        try:
            return json.loads(raw.decode("utf-8"))
        except json.JSONDecodeError as e:
            logger.error(f"JSON invalido en {filename}: {e}")
    return None
...
def list_all_available_samples() -> List[Dict]:
    """
    Construye la lista de muestras disponibles emparejando JSON y audio principal.
    Retorna lista de dicts con: json_file, wav_file, ecg_file, ecg1_file, ecg2_file.
    """
...
def upload_file(
    ...
    Usa HTTP PUT (requiere WebDAV o modulo dav en Apache).
    """
    url = f"{LOCAL_SERVER_BASE_URL}/{folder}/{filename}"
    try:
        resp = requests.put(
        ...
        logger.error(f"Excepcion subiendo {url}: {e}")
        return False
...
def upload_training_sample(
    ...
    wav_ecg: Optional[bytes] = None,
    wav_ecg_name: Optional[str] = None,
    wav_ecg1: Optional[bytes] = None,
    wav_ecg1_name: Optional[str] = None,
    wav_ecg2: Optional[bytes] = None,
